...
class IdleScreenRecorder:
    def __init__(
        ...
        self.use_input_listeners = use_input_listeners
    ...
    def _start_input_listeners(self) -> None:
        if not self.use_input_listeners:
            return
        mouse.Listener(
    ...
    def record_until_idle(self) -> None:
        ...
        while True:
            ...
            if prev_gray is not None:
                ...
                changed_ratio = (diff > self.pixel_threshold).mean()
                if changed_ratio > self.change_ratio_threshold:
                    self._mark_activity()
            prev_gray = gray

            # Idle and max-duration checks
            with self._activity_lock:
